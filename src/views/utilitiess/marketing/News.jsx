...
//                 InputProps={{
//                   endAdornment: (
//                     <Button variant="contained" onClick={onFileUpload} sx={{
//                       backgroundColor: "#03045E",
//                       '&:hover': {
//                         backgroundColor: "#03045E",
//                         opacity: 0.9
    ...
    try {
      ...
      if (fetchedData) {
        ...
        const tableData = sortedData.map((p, index) => ({
          newsId: index + 1, // ✅ Generate serial number instead of DB id
          originalId: p.newsId, // ✅ Keep actual DB id for edit/delete
          newsName: p.newsName,
          description: p.description,
          photoPath: p.photoPath, // ✅ Keep photoPath for card view
          file: p.photoPath ? <img src={ImageUrl + p.photoPath} alt={p.photoName} style={{ width: 100, height: 50 }} /> : 'NO IMAGE FOUND',
          insertedDate: moment(p.insertedDate).format('L'),
          updatedDate: moment(p.updatedDate).format('L'),
          createdBy: p.createdBy ? p.createdBy.userName : 'No User',
  ...
  return (
    ...
    >
      ...
      {viewMode === 'list' ? (
                  ...
                  <TableRow hover role="checkbox" tabIndex={-1} key={row.newsId}>
                    {columns.map((column) => (
                      <TableCell key={column.id} align={column.align}>
                        {column.id === 'actions' ? (
                          <>
                            <IconButton onClick={() => handleEdit(row.originalId)} color="primary">
                              <Edit />
                            </IconButton>
        ...
        // ✅ Use NewsCards component for card view
        <NewsCards
          news={news}
          page={page}
          rowsPerPage={rowsPerPage}
