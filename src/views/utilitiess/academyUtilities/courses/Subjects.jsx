...
// const validate = () => {
//   const newErrors = {};
//     if (!userdata.courseId.trim()) newErrors.courseId = 'Course is required';
...
//         courseCourseDto: { courseId: userdata.courseId },
...
const Subjects = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const tableData = fetchedData.map(p => ({
        subjectId: p.subjectId,
        subjectName: p.subjectName,
        courseName: p.courseCourseDto?.courseName || 'No Course',
        description: p.description,
        semester: Array.isArray(p.semester) ? p.semester.join(', ') : p.semester,
        url: p.url || '',
        videoUrl: p.videoUrl || '',
        fileName: p.fileName || '',
        filePath: p.filePath,
  ...
  const validate = () => {
    const newErrors = {};
    if (!subjectData.courseId) newErrors.courseId = 'Course is required';
    if (!subjectData.subjectName.trim()) newErrors.subjectName = 'Subject name is required';
    if (!subjectData.description.trim()) newErrors.description = 'Description is required';
    if (subjectData.semester.length === 0) newErrors.semester = 'At least one semester is required';
    return newErrors;
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        ...
        url: subjectData.url,
        videoUrl: subjectData.videoUrl,
        fileName: subjectData.fileName,
        filePath: subjectData.filePath,
        courseCourseDto: { courseId: subjectData.courseId },
        createdBy: editMode ? undefined : userInfo,
        updatedBy: editMode ? userInfo : undefined,
  ...
  const handleEdit = (subject) => {
    setSubjectId(subject.subjectId);
    setSubjectData({ 
      courseId: courses.find(c => c.courseName === subject.courseName)?.courseId || '',
      subjectName: subject.subjectName, 
      description: subject.description,
      semester: subject.semester.split(',').map(s => parseInt(s.trim())),
      url: subject.url,
      videoUrl: subject.videoUrl,
      fileName: subject.fileName,
      filePath: subject.filePath
  ...
  const handleSemesterChange = (e) => {
    ...
    setSubjectData(prev => ({
      ...prev,
      semester: typeof value === 'string' ? value.split(',').map(s => parseInt(s.trim())) : value
    }));
  };
    ...
    <MenuItem key={semester} value={semester}>
      {semester}
          ...
          <DialogActions sx={{ mt: 2 }}>
            <Button onClick={handleCloseDialog} sx={{ color: "#03045E" }}>
              Cancel
            </Button>
            <Button
              type="submit"
