  ...
  const FetchData = async () => {
    try {
      ...
      if (fetchedData) {
        ...
        const sortedData = [...fetchedData].sort((a, b) => a.universityId - b.universityId);
        
        const tableData = sortedData.map((p, index) => ({
          serialNo: index + 1, // This will create a sequential number
          universityId: p.universityId, // Keep for internal operations
          universityName: p.universityName,
          description: p.description,
          file:
            p.universityPicPath === null ? (
